...
#
# PURPOSE
#   Build multi-panel time-series plots for model trajectories and (optionally)
#   overlay observed data points. This function is used by both the static
#   plotting scripts and the Shiny app.
################################
...
plot_timeseries <- function(
  ...
  # -------------------------------------------------------------------------
  # Overall structure:
  #   1) Define color/shape/linetype palettes.
  #   2) Harmonize dose labels and factor ordering.
  #   3) Compute y-axis ranges for each variable.
  #   4) Build per-variable panels and combine with patchwork.
  # -------------------------------------------------------------------------
  ## -------------------------------------------------------------------------
  ...
  combined_plot <- (p_uc | p_ic | p_vir) /
    (p_inn | p_ada | p_sym) /
    (p_ad | p_ac | p_at) +
    # (p_cu | p_fV | p_fF) + #just used for diagnostics
