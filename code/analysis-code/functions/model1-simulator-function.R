...
# initial conditions and parameter values, as well as drug treatment information
# and time are passed in to the function
# the function returns a time-series with the solution of the ODE
# it needs: deSolve
#
# --------------------------------------------------------------------------------
# QUICK REFERENCE: STATE VARIABLES
#   Ad  = drug amount in depot
#   Ac  = drug amount in central compartment
#   At  = drug amount in target compartment
#   U   = uninfected cells
#   I   = infected cells
#   V   = free virus
#   F   = innate immune response proxy (e.g., IL-6)
#   A   = adaptive immune response proxy
#   S   = symptom/morbidity proxy (e.g., weight loss)
#
# QUICK REFERENCE: KEY PARAMETERS (examples)
#   b, k, p, kF, cV, gF, hV, Fmax, hF, gS, cS  = infection/immune params
#   Emax_V, C50_V, Emax_F, C50_F               = PD effect parameters
...
model1_simulator <- function(Ad, Ac, At, U, I, V, F, A, S, 
                           ...
                           Ad0, txstart, txinterval, txend, tstart, tfinal, dt, solvertype, tols)
 {
  ...
  # ---------------------------------------------------------------------------
  # Main part of the function: setup and solve the ODE system
  # ---------------------------------------------------------------------------
  ...
  # Times at which drug is administered (in days).
  drugtimes = seq(txstart, txend, by = txinterval)
