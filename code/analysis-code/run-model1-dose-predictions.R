...
source(here("code/analysis-code/model1-dose-predictions-function.R"))
...
simres_list <- future_lapply(
  ...
  function(i) {
    ...
    simulate_dose_predictions(bestfit, ts_doses = timeseries_doses, solvertype = solvertype, 
                              tols = tols, dt = dt, tfinal = tfinal, simulatorname = simulatorname)
  },
  future.seed = TRUE
)
...
{
  simres_list[[1]] <- simulate_dose_predictions(bestfit_list[[1]], ts_doses = timeseries_doses, 
                                                solvertype = solvertype, tols = tols, dt = dt, tfinal = tfinal, simulatorname = simulatorname)
}
...
saveRDS(
  simres_list,
  here::here('results', 'output', 'model1-dose-response-results.Rds')
)
