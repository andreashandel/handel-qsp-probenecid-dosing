#############################################################################
# wrapper to run dose prediction simulations for all posterior samples
#############################################################################
library(here)
...
simres_list <- future_lapply(seq_len(nsamp), function(i) {
...
saveRDS(simres_list,here::here('results','output','dose-response-results.Rds'))
