...
Our pharmacokinetic model for probenecid includes some of the known drug characteristics, while also keeping the overall model parsimonious. We model the 3 physiologically relevant sites, namely the oral depot, central compartment, and peripheral compartment (respiratory tract and lung). 
...
While the total concentration of the drug in the target compartment is given by $C_t = A_t/V_t$, probenecid binds to albumin, and only the unbound fraction of the drug, $C_u = f_u C_t$ is assumed to have a pharmacodynamic effect. For the central/plasma compartment, it was shown that the unbound fraction is concentration dependent [@emanuelsson1987; @emanuelsson1988]. A similar pattern is expected to hold for the target respiratory/lung compartment, however with a higher level of unbound drug, due to interstitial albumin concentration being around 20–30% of that in the plasma [@ellmerer2000]. To capture the reported concentration dependent binding pattern, we follow [@emanuelsson1987; @emanuelsson1988] and specify the fraction unbound as $f_u =  \frac{f_{max} C_{t}}{f_{50} + C_{t}}$, with $f_{max}$ the maximum fraction of unbound drug, and $f_{50}$ is the drug concentration at which binding is half-maximum. Values for rats reported in [@emanuelsson1988] are $f_{max} = 0.5$ and $f_{50} = 120 \mu g / mL$ for the plasma compartment. While a higher value for $f_{max}$ is expected for the target compartment, we decided to use the reported value as a conservative value as a lower bound for the expected free drug concentration at the target site.  


The PD functions describing the two effects are given by the following equations:
