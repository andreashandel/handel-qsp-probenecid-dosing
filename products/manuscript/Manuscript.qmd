...
$$
\textrm{NLL}_k =  \sum_{i,j} \left( \log(\sigma_k) + \frac{  (D_{i,j,k} - M_{i,j,k}) }{ \sigma_k }\right)^2
$$

The terms $\sigma_k$ are the variances for each variable, $D_{i,j,k}$ is the data, i.e. the measured value $i$ in treatment condition $j$ for variable $k$, and $M_{i,j,k}$ are the model predictions. 
