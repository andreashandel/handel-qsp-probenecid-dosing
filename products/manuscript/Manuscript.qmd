...
Data came from a recently published study on H5N1 influenza infections in mice [@murray2024]. In brief, female BALB/c mice were infected with 3 LD50 of H5N1 influenza virus. Animals either received no treatment or were given probenecid at a dosage of 10mg/kg or 100mg/kg (with an average mouse weight of 20g) twice daily by gavage for 3 days, starting on day 1 post infection. Viral loads, cytokine levels, and weight loss were measured on various days post infection. The data used includes viral load, body weight (as a measure of symptoms), and innate immune response (specifically, IL-6). For more details on the data, please refer to the original publication.
...
Since any within-host mechanistic model of influenza infection will need to include simplifications and abstractions of the detailed and complex processes surrounding interactions between virus, host cells, and the immune response, we decided to implement two alternative model structures. This allows some assessment of the dependence of results on model details.

Both models contain the same main compoments, namely uninfected cells and infected cells, virus, the innate immune response, the adaptive immune response, and symptoms/morbidity. 

The differences between the models are that model 2 includes a latent/ecclipse phase for infected cells and assumes that induction of innate response, adaptive response and symptoms depend on the log of the virus load. This contrasts to model 1 where virus (on the natural scale) is only assumed to impact innate response induction.

The models follow similar ones previously used by us and others to describe the dynamics of influenza infections [@handel2010; @handel2018; @canini14a; @dobrovolny13; @handel2018b; @beauchemin11; @kamal15].

 The models are illustrated schematically in @fig-model and given mathematically by the following set of ordinary differential equations:
...
\textrm{Adaptive response:} \quad \dot A & = \frac{F}{F+h_F} + g_A A \\ 
\textrm{Symptoms:} \quad \dot S & = g_S F - c_S S \\ 
\end{aligned}
$$ {#eq-qsp1}

The second model is given by the following set of equations and shown schematically in @fig-model B:

$$
\begin{aligned}
\textrm{Uninfected cells:} \quad \dot U & = -bUV \\
\textrm{Latently infected cells:} \quad \dot E & = bUV - c_E E\\
\textrm{Productviely infected cells:} \quad \dot I & = c_E E - c_I I - k IA\\
\textrm{Virus:} \quad \dot V & = (1 - f_V)\frac{p}{ (1 + k_F F)}I - c_V V \\
\textrm{Innate response:} \quad \dot F & = (1 - f_F)\frac{g_F V_l}{V_l + h_V} (F_{max}-F) - c_F F \\
\textrm{Adaptive response:} \quad \dot A & = \frac{F V_l}{F V_l + h_F} + g_A A \\ 
\textrm{Symptoms:} \quad \dot S & = g_S F V_l  - c_S S \\ 
\end{aligned}
$$ {#eq-qsp2}
...
Some of the PK and PD parameters were fixed as described above. To further reduce overfitting, additional prior information from previously published studies was used to fix selected parameters of the QSP model. Specifically, $c_F = 1$/day was set based on the reported decay rate of IL-6 [@kuribayashi2018], the T-cell growth rate was fixed at $g_A = 3$/day [@lewis2021], the duration of the latent phase for infected cells was set to 6 hours ($c_E = 4$/day$) [@beauchemin11], and the clearance rate of infected cells was set to $c_I = 1$/day [@smith11].

To estimate the remaining parameters, the model predictions for log viral load ($\log_10(V+1)$), innate response, and symptoms were fit to the data for log virus load, IL6 and percent weight loss from the H5N1 mouse study. Since most measurements involved destructive sampling (one mouse per timepoint), and identifiers for specific animals were not recorded, individual level information is unavailable, which precludes hierarchical modeling approaches that make use of individual-level time-series measurements. Instead, a maximum likelihood approach was used to fit the model to the data. The model was simultaneously fit to all three variables across the three treatment conditions (untreated, 10 mg, 100 mg). A normal distribution with additive errors and independent variances for the three fitted quantities was assumed. Proportional error terms were explored but did not improve results and were thus fixed to zero.

Since sample size across measured quantities was uneven, we weighted each observation by the inverse total sample size for that variable (across all treatment conditions) so that virus load, IL‑6, and weight loss contribute equally to the objective.


This resulted in the following objective function being minimized by the fitting routine:

$$
O = \sum_{j} \frac{1}{N_{j}} \sum_k \textrm{NLL}_{j,k}  
$$

The indices $j$ and $k$ stand for the 3 different variables (virus load, innate response, symptoms) and the 3 treatment conditions (untreated, 10 mg, 100 mg), respectively.

$N_{j} = \sum_k N_{j,k}$ is the number of data points for each variable $j$ across all treatment conditions, and $\textrm{NLL}_{j,k}$ is the negative log-likelihood term for each variable, given by

$$
\textrm{NLL}_{j,k} = 0.5 \sum_{i} \left( \log(\sigma_j^2) + \left( \frac{  D_{i,j,k} - M_{i,j,k} }{ \sigma_j }\right)^2 \right)
$$

The terms $\sigma_j^2$ are the variances for each variable, which were assumed constant across treatment conditions and fixed from the empirical variance of each variable on its fitting scale. $D_{i,j,k}$ is the data, i.e. the measured value $i$ of variable $j$ in treatment condition $k$, and $M_{i,j,k}$ are the model predictions. 
...
#| echo: FALSE
knitr::include_graphics(here("results", "figures", "model1-bestfit1.png"))
```
...
knitr::include_graphics(here(
  "results",
  "figures",
  "model1-dose-response-baseline.png"
))
```
...
#| echo: FALSE
knitr::include_graphics(here("results", "figures", "model1-dose-response-txstart.png"))
```
...
knitr::include_graphics(here(
  "results",
  "figures",
  "model1-dose-response-txinterval.png"
))
```
