...
Since any within-host mechanistic model of influenza infection will need to include simplifications and abstractions of the detailed and complex processes surrounding interactions between virus, host cells, and the immune response, we decided to implement two alternative model structures. This allows some assessment of the robustness of our findings to model structure.

Both models contain the same main compoments, namely uninfected cells and infected cells, virus, the innate immune response, the adaptive immune response, and symptoms/morbidity. The models follow similar ones previously used by us and others to describe the dynamics of influenza infections [@handel2010; @handel2018; @canini14a; @dobrovolny13; @handel2018b; @beauchemin11; @kamal15].

The differences between the models are that model 2 includes a latent/ecclipse phase for infected cells and assumes that adaptive response and symptom induction rates are dependend on both innate response strength and virus load, versus just innate response in model 1.


 The first model is illustrated schematically in @fig-model A and given mathematically by the following set of ordinary differential equations:
...
\textrm{Productviely infected cells:} \quad \dot I & = c_E E - k IA\\
\textrm{Virus:} \quad \dot V & = (1 - f_V)\frac{p}{ (1 + k_F F)}I - c_V V \\
\textrm{Innate response:} \quad \dot F & = (1 - f_F)\frac{g_F V_l}{V_l + h_V} (F_{max}-F) - c_F F \\
...
$$
O = \sum_{j,k} \frac{1}{N_{j,k}}  \textrm{NLL}_{j,k}  
$$
...
$N_{j,k}$ is the number of data points for each variable and treatment condition, and $\textrm{NLL}_{j,k}$ is the negative log-likelihood term for each variable, given by

$$
\textrm{NLL}_{j,k} = 0.5 \sum_{i} \left( \log(\sigma_j^2) + \left( \frac{  D_{i,j,k} - M_{i,j,k} }{ \sigma_j }\right)^2 \right)
$$

The terms $\sigma_j^2$ are the variances for each variable, which are assumed to be the same across treatment conditions and fixed based on the variance in the data for each variable. $D_{i,j,k}$ is the data, i.e. the measured value $i$ of variable $j$ in treatment condition $k$, and $M_{i,j,k}$ are the model predictions. 
