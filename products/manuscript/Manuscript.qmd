...
\textrm{Virus:} \quad \dot V & = (1 - f_V)\frac{p}{ (1 + k_F F)}I - c_V V \\
\textrm{Innate response:} \quad \dot F & = (1 - f_F)\frac{g_F V}{V+h_V} (F_{max}-F) - c_F F \\
\textrm{Adaptive response:} \quad \dot A & = \frac{F \log(V)}{F \log(V) + h_F} + g_A A \\ 
\textrm{Symptoms:} \quad \dot S & = g_S F \log(V) - c_S S \\ 
\end{aligned}
$$ {#eq-qsp2}
...
The terms $\sigma_j^2$ are the variances for each variable, which are assumed to be the same across treatment conditions. $D_{i,j,k}$ is the data, i.e. the measured value $i$ of variable $j$ in treatment condition $k$, and $M_{i,j,k}$ are the model predictions. 
